...
const TaggedArticleList = ({ data, pageContext }: PageProps<Queries.TaggedArticleListQueryQuery, TaggedArticlesPageContext>) => {
    const [isLarge] = useMediaQuery([
        "(min-width: 1280px)",
    ])

    const articleCardDataList: ArticleCardData[] = (() => {
        return data.allMarkdownRemark.nodes.map((node): ArticleCardData|undefined => {
            const frontmatter = node.frontmatter
            if (!frontmatter) {
                return undefined
            }

            const imageDataEdge = data.allFile.edges.find((edge: { node: { id: string; }; }) => edge.node.id === `ArticleImage:${frontmatter.id}`);
            ...
            const articleCardData: ArticleCardData = {
                ...
                createdAt: createdAt,
                updatedAt: updatedAt,
                tags: tags?.filter((tag) => tag !== undefined && tag !== null)
                .map((tag) => { return {id: tag.id ?? "", name: tag.name ?? ""} }) ?? Array.of<Tag>(),
                imageData: imageData,
    ...
    return (
        <Layout scroll={true} isLarge={isLarge}>
            <main>
                <Heading className={"text-black text-3xl font-bold"} paddingBottom={"md"}>#{pageContext.tagName}</Heading>
