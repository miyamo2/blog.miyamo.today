...
export const ArticleCard: FC<VerticalArticleCardProps> = (props: VerticalArticleCardProps) => {
    const gatsbyImage = (() => {
        ...
        return image ? <GatsbyImage image={image} alt={`ArticleImage:${props.id}`} objectFit={"cover"} className={"transform-scaleup-then-hover"}/>: <></>
    })()

    const createdAt = format(new Date(props.createdAt), "YYYY/MM/DD")

    return (
        <Grid rounded={"xl"} boxShadow={"md"} templateRows={"subgrid"} gridRow={"span 6"} gap={"sm"} padding={"sm"} as={Link} to={`/articles/${props.id}`}>
                ...
                <GridItem><Text color="muted">{props.articleExcerpt}</Text></GridItem>
                <GridItem>
                    <Divider/>
                </GridItem>
                <GridItem>
                    <Text><FontAwesomeIcon icon={faCalendarDay} />{createdAt}</Text>
                </GridItem>
