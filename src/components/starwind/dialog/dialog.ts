/*
 * The starwind dialog handler, lifted out of Dialog.astro's `<script>`.
 *
 * As a component script it was a module chunk in the critical request chain of
 * every page -- the largest same-origin one after the client router -- even
 * though nothing on the page needs a dialog until someone reaches for the
 * search button, the hamburger menu or the article TOC button. Layout.astro's
 * bootstrap imports this on the first sign of that intent instead, and calls
 * `setupDialogs()` itself; the listeners below then keep the dialogs wired for
 * the rest of the session, exactly as the component script used to.
 */
...
class DialogHandler {
  ...
  private setupEvents(): void {
    ...
    this.dialog.addEventListener("click", (e) => {
      if (!this.dialog) return;
      if (e.target instanceof Node && e.target !== this.dialog && this.dialog.contains(e.target)) {
        return;
      }
  ...
  private open(): void {
    ...
    // Prevent the browser's default autofocus on the first focusable element
    // inside the dialog (which would show an unwanted focus-visible ring).
    this.dialog.focus();
...
export const setupDialogs = (): void => {
